...
const Profile = () => {
     ...
     const { userData , BACKEND_URI ,setUserData } = useContext(UserContext)


     const submit = async (data) => {
          console.log(data)
          const response = await axios.put(`${BACKEND_URI}user/update/${userData._id}`,data)
     ...
     return (
          <div className='w-full h-full flex items-center justify-center text-zinc-200 relative flex-col gap-y-10 py-5'>
               <div className='md:w-[50%] w-[90%] md:h-[80%] h-[90%] border-2 rounded-3xl flex flex-col border-zinc-800 justify-center items-center py-5 '>
                    <p className='text-3xl py-5 h-[10%] font-medium'>Profile</p>
                    <form onSubmit={handleSubmit(submit)} className='w-full h-[80%] flex-col flex gap-6 items-center '>
